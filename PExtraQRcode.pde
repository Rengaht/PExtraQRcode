...
void draw(){
  ...
  // draw last 2 qrcode
  for(int i=0;i<2;++i){
    if(_img_sticker[i]!=null) image(_img_sticker[i],width/2-_qrwid*1.2,height/2-_qrwid+_qrwid*1.2*i,_qrwid,_qrwid);    
    if(_img_qrcode[i]!=null) image(_img_qrcode[i],width/2+_qrwid*.2,height/2-_qrwid+_qrwid*1.2*i,_qrwid,_qrwid);
    if(_img_id[i]!=null){
      pushStyle();
        fill(0);
        text(_img_id[i],width/2+_qrwid/2,height/2-_qrwid+_qrwid*1.3*i);
      popStyle();
    }
  }
